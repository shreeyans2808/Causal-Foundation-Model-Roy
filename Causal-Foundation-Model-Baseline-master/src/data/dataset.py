...
class TestDataset(MetaDataset):
    ...
    def __getitem__(self, idx):
        ...
        print(f"DEBUG: Processing dataset {idx}, num_batches={num_batches}")
        print(f"DEBUG: Dataset key={dataset.key}, algorithm={dataset.algorithm}")
        start = time.time()  # keep track of CPU time
        batches, corrs = self._sample_batches(dataset, num_batches)
        print(f"DEBUG: Generated {len(batches)} batches, {len(corrs)} correlations")
        # learned sampler = we already ran the algorithms
        if self.args.use_learned_sampler:
            graphs, orders = self.graphs, self.orders
        else:
            results = self.run_alg(batches)
            print(f"DEBUG: Algorithm results: {len(results) if results else 0} results")
            graphs, orders = convert_to_graphs(results, dataset)
            print(f"DEBUG: Converted graphs: {graphs.shape if graphs is not None else 'None'}")
        end = time.time()  # keep track of CPU time
        dataset.time = end - start
        if graphs is None:
            print("DEBUG: Graphs is None, returning empty dict")
            return {}
        result = convert_to_item(dataset, corrs, graphs, orders)
