...
def run_gies(batch, regime):
    try:
        ...
        # Convert regime to list format (GIES expects list of lists for interventions)
        if regime is not None:
            if isinstance(regime, np.ndarray):
                regime = regime.tolist()
        print(f"DEBUG GIES: batch shape={batch.shape}, regime type={type(regime)}, len={len(regime) if regime is not None else 'None'}")
        graph, score = fit_bic(data=batch, I=regime, A0=None,
                phases=["forward", "backward", "turning"],
                iterate=True, debug=0)  # a real verbose flag!
        print(f"DEBUG GIES: Success! Graph shape={graph.shape}")
    except Exception as e:
        print(f"DEBUG GIES: Error occurred: {type(e).__name__}: {e}")
        import traceback
        traceback.print_exc()
        return
...
def collate(args, batch):
    ...
    for key, val in batch.items():
        # Debug: check if val is empty
        if len(val) == 0:
            print(f"Warning: Empty batch for key '{key}', skipping...")
            continue
