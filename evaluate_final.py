...
def evaluate_causal_discovery():
    ...
    if len(predictions_flat) == 9900:
        ...
        n_nodes = 100
        print(f"Detected off-diagonal adjacency matrix format ({n_nodes}x{n_nodes} - diagonal)")
        ...
        print(f"Detected full adjacency matrix format ({n_nodes}x{n_nodes})")
    ...
    for i, idx in enumerate(flat_indices[::-1]):  # Show in descending order
        if len(predictions_flat) == 9900:
            # Convert flattened off-diagonal index to matrix coordinates
            mask_flat = (~np.eye(n_nodes, dtype=bool)).flatten()
            matrix_indices = np.arange(n_nodes * n_nodes)[mask_flat]
            matrix_idx = matrix_indices[idx]
            row, col = matrix_idx // n_nodes, matrix_idx % n_nodes
        else:
